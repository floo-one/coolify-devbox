...
**B) Public HTTPS URL** (reachable anywhere, incl. phone; optionally password-protected)
Set the app's **Domains** field to `https://<your-domain>:9009` and mirror the URL in the
`DEVBOX_URL` env var (see [Setup](#setup)). The box fronts your dev server with Caddy:

```
your dev server on 127.0.0.1:3000  ──►  Caddy on :9009  ──►  your domain (HTTPS via Coolify proxy)
```
...
- **Auth**: set `DEVBOX_AUTH=user:password` to gate the URL with basic auth; leave it empty to
  serve it public. The entrypoint hashes the password at runtime — **nothing is committed to
  this repo.** Rotate by changing `DEVBOX_AUTH` and restarting.
...
| `docker-compose.yaml` | Publishes SSH on `DEVBOX_SSH_PORT` (default 2222), plain `DEVBOX_*` env vars (no Coolify magic vars — see comments in the file), mounts the `devhome` volume, healthchecks sshd. (`.yaml` — matches Coolify's default compose location.) |
...
| `DEVBOX_URL` | for the public URL | The dev URL, mirroring the Domains field without the port — e.g. `https://dev.example.com`. Exported to shells as `DEVBOX_URL`/`DEVBOX_FQDN`. |
| `DEVBOX_AUTH` | optional | `user:password` gates the public URL with basic auth; **empty = public**. Malformed values fail closed (URL disabled). |
...
**Why no `SERVICE_FQDN_*`/`SERVICE_PASSWORD_*` magic vars?** They're designed for Coolify's
one-click service templates; on git-based compose apps they cause domain churn and a phantom
"configuration changed — redeploy required" warning on every deploy. Setting the Domains field
yourself (once) plus plain env vars is boring and reliable. The `:9009` in the Domains field
matters — the container also exposes port 22, so Traefik can't infer the port. Visitors never
see it; the shared link is just `https://<your-domain>`.
...
3. Set env vars: `AUTHORIZED_KEYS` = your SSH public key (required). For the public dev URL, set
   the app's **Domains** field to `https://<your-domain>:9009`, set `DEVBOX_URL=https://<your-domain>`,
   and (recommended) `DEVBOX_AUTH=user:somepassword`. Deploy.
...
2. Set env vars:
   ...
   - For the public dev URL: set the app's Domains field to https://<my-chosen-domain>:9009,
     set DEVBOX_URL=https://<my-chosen-domain>, and DEVBOX_AUTH=<user:password you generate>.
     Skip all three if I only want port-forward previews.
